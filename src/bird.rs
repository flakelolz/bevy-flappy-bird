...
impl Plugin for BirdPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(BirdAssets::default())
            .add_systems(
                Startup,
                (load_bird_image, load_bird_atlas, spawn_bird)
                    .chain()
...
fn load_bird_image(asset_server: Res<AssetServer>, mut birds: ResMut<BirdAssets>) {
    birds.birds = Some(asset_server.load("sprites/birds/flappybird-sheet.png"));
}
...
fn load_bird_atlas(
    ...
    let texture_handle = bird_assets.birds.as_ref().expect("Embedded in the binary");
...
fn spawn_bird(mut commands: Commands, bird_assets: ResMut<BirdAssets>) {
    ...
    commands.spawn((
        SpriteSheetBundle {
            texture_atlas: bird_assets.birds_atlas.clone().expect("Embedded"),
